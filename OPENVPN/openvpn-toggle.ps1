...
===============================================================================
 OpenVPN Toggle
-------------------------------------------------------------------------------
 Controla o OpenVPN no Windows: servicos + adaptadores TAP.

 Mode Enable:
   1) Desbloqueia scripts da pasta (remove Zone.Identifier)
   2) Define servicos como "demand" (inicio manual)
   3) Inicia servicos OpenVPN
   4) Habilita adaptadores TAP / OpenVPN
   5) Abre OpenVPN GUI

 Mode Disable:
   1) Para servicos OpenVPN (aguarda confirmacao real de Stopped)
   2) Define servicos como "disabled"
   3) Desabilita adaptadores TAP / OpenVPN
...
try {
    ...
    $LogDir = Join-Path $env:TEMP 'openvpn_toggle_logs'
    if (-not (Test-Path $LogDir)) { New-Item -ItemType Directory -Path $LogDir -Force | Out-Null }
}
...
# -----------------------------------------------------------------------
# Deteccao de servicos OpenVPN
# OpenVPNService           : servico principal (conexoes automaticas via config)
# OpenVPNServiceInteractive: servico interativo usado pela GUI
# -----------------------------------------------------------------------
$knownNames = @(
    'OpenVPNService',              # OpenVPN Service
    'OpenVPNServiceInteractive'    # OpenVPN Interactive Service (necessario para GUI)
)
...
$extra = Get-Service -ErrorAction SilentlyContinue |
         Where-Object {
             $_.DisplayName -like '*OpenVPN*' -and
...
if ($present.Count -eq 0) {
    Write-Log 'Nenhum servico OpenVPN encontrado. Verifique se o OpenVPN esta instalado.' 'Red'
...
function Toggle-OpenVPNAdapters {
    param([ValidateSet('Enable', 'Disable')][string]$Action)

    # Adaptadores TAP-Windows sao instalados pelo OpenVPN.
    # Tambem captura adaptadores com "OpenVPN" no nome (ex: OpenVPN Connect).
    ...
    if (-not $adapters) {
        Write-Log 'Nenhum adaptador TAP/OpenVPN encontrado (normal se OpenVPN nao esta instalado).' 'DarkGray'
        return
    }
...
# -----------------------------------------------------------------------
# MODE: ENABLE
# OpenVPNServiceInteractive deve iniciar antes da GUI ser aberta
# -----------------------------------------------------------------------
if ($Mode -eq 'Enable') {

    Write-Log '--- [1/3] Iniciando servicos OpenVPN ---' 'Cyan'
    ...
    Write-Log '--- [2/3] Habilitando adaptadores TAP/OpenVPN ---' 'Cyan'
    Start-Sleep -Seconds 2
    Toggle-OpenVPNAdapters -Action Enable

    Write-Log '--- [3/3] Abrindo OpenVPN GUI ---' 'Cyan'
    $guiPaths = @(
    ...
    if ($gui) {
        Write-Log "Abrindo OpenVPN: $gui" 'Cyan'
        try { Start-Process -FilePath $gui -ErrorAction Stop }
        catch { Write-Log "Falha ao abrir OpenVPN GUI: $($_.Exception.Message)" 'Red' }
    } else {
        Write-Log 'OpenVPN GUI nao encontrada nos caminhos padrao.' 'Yellow'
    }

    Write-Log '--- OpenVPN habilitado. ---' 'Green'
}
