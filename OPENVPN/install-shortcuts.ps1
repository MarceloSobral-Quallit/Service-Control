<#
===============================================================================
 OpenVPN Toggle - Instalador de Atalhos
-------------------------------------------------------------------------------
 Cria atalhos no Menu Iniciar (Service Control) que chamam openvpn-toggle.ps1.

 Parametros:
   -DisableAllNow   Para e desabilita todos os servicos OpenVPN imediatamente.
...
$progDataDir = 'C:\ProgramData\ServiceControl\OPENVPN'
$togglePs1   = Join-Path $progDataDir 'openvpn-toggle.ps1'
...
if (-not (Test-Path $togglePs1)) {
    ...
    Write-Host 'Certifique-se de que openvpn-toggle.ps1 esta na mesma pasta.' -ForegroundColor Yellow
    exit 1
}
...
New-Shortcut `
    -LnkPath   (Join-Path $targetFolder 'OpenVPN - Enable.lnk') `
    -Target    'powershell.exe' `
    -Arguments "-NoProfile -ExecutionPolicy Bypass -File `"$togglePs1`" -Mode Enable" `
    -Desc      'Habilita servicos e adaptadores OpenVPN e abre GUI'
...
New-Shortcut `
    -LnkPath   (Join-Path $targetFolder 'OpenVPN - Disable.lnk') `
    -Target    'powershell.exe' `
    -Arguments "-NoProfile -ExecutionPolicy Bypass -File `"$togglePs1`" -Mode Disable" `
    -Desc      'Para e desabilita todos os servicos e adaptadores TAP/OpenVPN'
...
New-Shortcut `
    -LnkPath   (Join-Path $targetFolder 'OpenVPN - Abrir pasta de logs.lnk') `
    -Target    'explorer.exe' `
    -Arguments "`"$logsDir`"" `
    -IconPath  'imageres.dll,3' `
    -Desc      'Abre pasta de logs do OpenVPN Toggle'
...
Get-ChildItem -Path $targetFolder -Filter 'OpenVPN - *.lnk' -ErrorAction SilentlyContinue | ForEach-Object {
...
if ($DisableAllNow) {
    ...
    Write-Host "`n--- Desativando todos os servicos OpenVPN agora... ---" -ForegroundColor Yellow

    $svcNames = @('OpenVPNService', 'OpenVPNServiceInteractive')
    $present  = [System.Collections.Generic.List[string]]::new()
    foreach ($n in $svcNames) {
        $svc = Get-Service -Name $n -ErrorAction SilentlyContinue
        if ($svc) { $present.Add($svc.Name) }
    }
    $extra = Get-Service -ErrorAction SilentlyContinue |
             Where-Object { $_.DisplayName -like '*OpenVPN*' -and (-not $present.Contains($_.Name)) }
    foreach ($svc in $extra) { $present.Add($svc.Name) }

    if ($present.Count -eq 0) {
        Write-Host "Nenhum servico OpenVPN encontrado para desativar." -ForegroundColor DarkYellow
    } else {
    ...
    Write-Host "`n--- Desabilitando adaptadores TAP/OpenVPN agora... ---" -ForegroundColor Yellow
    ...
    if (-not $adapters) {
        Write-Host "  Nenhum adaptador TAP/OpenVPN encontrado." -ForegroundColor DarkGray
    } else {
    ...
    Write-Host "`nPronto. Use o atalho 'Enable' quando precisar do OpenVPN." -ForegroundColor Green
}

# -----------------------------------------------------------------------
# Tarefa agendada: desabilita adaptadores e servicos OpenVPN a cada boot
...
$bootTaskName = 'ServiceControl_OpenVPN_DisableAdaptersOnBoot'
...
try {
    Register-ScheduledTask `
        ...
        -Description 'Service Control: desabilita servicos e adaptadores OpenVPN a cada boot' `
